...
import fs from 'fs';
import { ActivityBar, By, EditorView, InputBox, SideBarView, TreeItem, until, ViewControl, ViewPanelActionDropdown, ViewSection, VSBrowser, WebDriver, WebElement } from "vscode-extension-tester";
...
describe('Integrations View', function () {
    ...
    (process.platform === 'darwin' ? describe.skip : describe)(`Click 'New File...' button`, function () {
        ...
        after(function () {
            fs.rmSync(join(WORKSPACE_FOLDER, CAMEL_ROUTE_FILE));
            fs.rmSync(join(WORKSPACE_FOLDER, 'kamelets', KAMELET_FILE));
            fs.rmSync(join(WORKSPACE_FOLDER, 'pipes', 'others', PIPE_FILE));
        });
        ...
        it(`Check new 'Pipe' can be created`, async function () {
            ...
            expect(newCamelRoute).to.not.be.undefined;
            expect(await newCamelRoute?.getDescription()).to.be.equal('pipes/others');
        ...
        async function getNewFileTreeItem(filename: string): Promise<TreeItem> {
            ...
            }, 90_000, `${filename} was not created properly within Integrations view!`);
        }
        ...
        async function switchToKaotoAndCheckIntegrationType(filename: string, type: string): Promise<void> {
            ...
            }, 90_000, `Kaoto editor for a new ${type} file was not opened!`);
